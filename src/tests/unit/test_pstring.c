...
TEST test_pact_string_clone_simple() {
	...
	ASSERT_EQm("after pact_string_clone, length on src incorrect", 23, pact_string_length(pstring));
	ASSERT_STR_EQm("after pact_string_clone, dst has incorrect value", str, pact_string_view(pstring2));
	ASSERT_STR_EQm("after pact_string_clone, src has incorrect value", str, pact_string_view(pstring));
	pact_string_destroy(pstring);
	pact_string_destroy(pstring2);
	PASS();
}
...
TEST test_pact_string_clone_nulls() {
	...
	ASSERTm("pact_string_clone returned non-zero", pact_string_clone(pstring, pstring2) == 0);
	ASSERT_EQm("after pact_string_clone, length mismatch", pact_string_length(pstring), pact_string_length(pstring2));
	ASSERT_EQm("after pact_string_clone, length on dst incorrect", 40, pact_string_length(pstring2));
	ASSERT_EQm("after pact_string_clone, length on src incorrect", 40, pact_string_length(pstring));
	ASSERTm("after pact_string_clone, dst has incorrect value", memcmp(str, pact_string_view(pstring2), 29) == 0);
	ASSERTm("after pact_string_clone, src has incorrect value", memcmp(str, pact_string_view(pstring), 29) == 0);
	pact_string_destroy(pstring);
	pact_string_destroy(pstring2);
	PASS();
}
...
TEST test_pact_string_clone_substr_start() {
	...
	ASSERT_STR_EQm("after pact_string_clone, dst has incorrect value", check, pact_string_view(pstring2));
	ASSERT_STR_EQm("after pact_string_clone, src has incorrect value", str, pact_string_view(pstring));
	pact_string_destroy(pstring);
	pact_string_destroy(pstring2);
	PASS();
